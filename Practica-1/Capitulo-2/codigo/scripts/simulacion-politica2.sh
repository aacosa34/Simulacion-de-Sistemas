...
echo "Veces,M-CostoTotal,M-CostoPedido,M-CostoMantenimiento,M-CostoDeficit" > ../../datos/simulacion-politicas.csv
...
for i in ${spequena[@]}; do
    for j in ${sgrande[@]}; do
        for k in ${nsimulaciones[@]}; do
            ../inventario $k $i $j >> ../../datos/simulacion-politicas.csv
        done
    done
done
