...
echo "Veces,M-CostoTotal,M-CostoPedido,M-CostoMantenimiento,M-CostoDeficit" > ../../datos/simulacion-politica3.csv
...
for i in ${spequena[@]}; do
    for j in ${sgrande[@]}; do
        for k in ${nsimulaciones[@]}; do
            ../inventario3 $k $i $j >> ../../datos/simulacion-politica3.csv
        done
    done
done